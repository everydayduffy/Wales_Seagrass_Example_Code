...
files <- tools::file_path_sans_ext(list.files("Data/Classified_RGB", pattern = ".*.txt"))

results <- NULL

for(h in 1:length(files))
{
  data <- read.table(paste0("Data/Classified_RGB/",files[h],".txt"),h=T)
  ...
  for(i in 1:length(combos))
  {
    ...
    for (j in 1:nrow(sub_combos))
    {
      ...
      write.table(out,paste0("Data/Classified_RGB_Combn/",files[h],"_com_",i,"_",j,".txt"),row.names=F)
    }
  }
}
